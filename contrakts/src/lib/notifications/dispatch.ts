...
export async function sendMilestoneSubmittedEmail({
  ...
  const { data: contract } = await supabaseAdmin
    .from('contracts')
    .select('id, title, currency, initiator_id, counterparty_id')
    .eq('id', contractId)
    .maybeSingle()
  ...
  const client = await getUserContact(contract.initiator_id)
  const vendor = await getUserContact(contract.counterparty_id)
