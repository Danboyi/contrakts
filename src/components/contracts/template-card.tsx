...
export function TemplateCard({
  ...
  return (
    <div
      className={cn(
        'group flex h-full w-full flex-col overflow-hidden rounded-[var(--radius-xl)] border border-[hsl(var(--color-border))] bg-[hsl(var(--color-surface))] p-5',
    ...
    >
      ...
      <Link href={`/templates/${template.id}`} className="mb-2 block">
        <h3 className="line-clamp-2 text-sm font-semibold leading-snug text-[hsl(var(--color-text-1))] transition-colors duration-150 hover:text-[hsl(var(--color-accent))]">
          {template.title}
        </h3>
      </Link>
      ...
      <div className="mb-4 mt-auto flex min-w-0 flex-wrap items-center gap-x-3 gap-y-2">
        ...
        <span className="min-w-0 text-xs text-[hsl(var(--color-text-3))]">
          {hasAmounts && totalHint > 0
            ? `~${formatCurrency(totalHint * 100, template.currency)} typical`
            : 'Custom'}
        </span>
        {template.use_count > 0 && (
          <span className="text-xs text-[hsl(var(--color-text-3))] sm:ml-auto">
