...
export function ContractPreview({
  ...
  const effectivePlatformFee = effectiveTotal * 0.02
  const effectiveNetToProvider = effectiveTotal - effectivePlatformFee
  const initiatorRole = data.initiator_role
  const counterpartyRole = initiatorRole
    ? getCounterpartyRole(initiatorRole)
    : null
  const counterpartyLabel =
    data.counterparty_name || data.counterparty_email || 'Not set yet'
  ...
  return (
    ...
    >
      ...
      <div className="p-5">
        ...
        {initiatorRole && (
          <div className="mb-4 inline-flex rounded-full border border-[hsl(var(--color-accent)/0.15)] bg-[hsl(var(--color-accent)/0.08)] px-3 py-1.5 text-[11px] font-medium text-[hsl(var(--color-accent))]">
            You are the {ROLE_LABELS[initiatorRole]}
          </div>
        )}

        <div className="mb-4 flex flex-col gap-3 rounded-[var(--radius-md)] bg-[hsl(var(--color-surface-2))] p-3 sm:flex-row sm:items-start">
          <div className="min-w-0 flex-1">
            <p className="mb-1 text-[10px] uppercase tracking-wide text-[hsl(var(--color-text-3))]">
              {initiatorRole ? ROLE_LABELS[initiatorRole] : 'Initiator'}
            </p>
            <p className="truncate text-sm font-medium text-[hsl(var(--color-text-1))]">
              {userName}
            </p>
          </div>
          <div className="h-px w-full bg-[hsl(var(--color-border))] sm:h-auto sm:w-px sm:self-stretch" />
          <div className="min-w-0 flex-1">
            <p className="mb-1 text-[10px] uppercase tracking-wide text-[hsl(var(--color-text-3))]">
              {counterpartyRole ? ROLE_LABELS[counterpartyRole] : 'Counterparty'}
            </p>
            <p
              className={cn(
                'truncate text-sm',
                data.counterparty_name || data.counterparty_email
            ...
            {data.counterparty_name && data.counterparty_email && (
              <p className="mt-1 truncate text-[11px] text-[hsl(var(--color-text-3))]">
        ...
        <div className="flex flex-col gap-2">
          <div className="flex items-center justify-between">
            ...
            <span className="text-sm font-semibold text-[hsl(var(--color-text-1))]">
              {formatCurrency(toSmall(effectiveTotal), currency)}
            </span>
          </div>
          <div className="flex items-center justify-between">
            <span className="text-xs text-[hsl(var(--color-text-3))]">
            ...
            <span className="text-sm text-[hsl(var(--color-warning))]">
              -{formatCurrency(toSmall(effectivePlatformFee), currency)}
            </span>
          </div>
          <div className="flex items-center justify-between border-t border-[hsl(var(--color-border))] pt-2">
            <span className="text-xs font-medium text-[hsl(var(--color-text-2))]">
              Net to service provider
            </span>
            <span className="text-base font-semibold text-[hsl(var(--color-success))]">
              {formatCurrency(toSmall(effectiveNetToProvider), currency)}
            </span>
          </div>
        </div>
