  ...
  .superRefine((value, ctx) => {
    if (value.ruling === 'partial' && value.rulingPctVendor === undefined) {
      ctx.addIssue({
        ...
        message: 'A service provider split percentage is required for a partial ruling.',
      })
    }
  })
...
async function queuePartialVendorPayment(params: {
  ...
  if (error || !payment) {
    return { error: 'Could not queue the service provider split payout.' }
  }
    ...
    await insertNotifications([
      {
        ...
        body: `The service provider portion of the dispute ruling for "${params.milestoneTitle}" is queued for manual review.`,
      },
      {
        user_id: params.counterpartyId,
        contract_id: params.contractId,
        type: 'split_payout_review',
        title: 'Service provider payout queued',
...
export async function issueRulingInternal(
  ...
  if (ruling === 'vendor_wins') {
    ...
    if (clientAmount > 0) {
      ...
      await insertNotifications({
        ...
        contract_id: contract.id,
        type: 'refund_review',
        title: 'Service receiver refund queued',
        body: `The service receiver portion of the dispute ruling for "${milestone.title}" is queued for manual handling.`,
      })
    }
  ...
  const rulingLabel = {
    vendor_wins: 'Ruling: service provider wins - payment release initiated.',
    client_wins: 'Ruling: service receiver wins - milestone reset for re-delivery.',
    partial: `Ruling: partial - ${rulingPctVendor}% allocated to the service provider.`,
    cancelled: 'The dispute was cancelled and returned to service receiver review.',
  }[ruling]
