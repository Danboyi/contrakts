...

{
  ...
  "evidence_summary": {
    ...
    "gap_analysis": "string - where service receiver and service provider disagree"
  },
  "contract_compliance": {
    "scope_met": boolean,
...
Service receiver's complaint: ${dispute.description}
...
${
  evidence.client.length > 0
    ? evidence.client
        ...
        .join('\n')
    : 'No evidence submitted by the service receiver.'
}
...
${
  evidence.vendor.length > 0
    ? evidence.vendor
        ...
        .join('\n')
    : 'No response submitted by the service provider.'
}
