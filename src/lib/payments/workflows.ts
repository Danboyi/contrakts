...
export async function markEscrowDepositSuccess({
  ...
  if (contract) {
    const { data: contractParties } = await supabaseAdmin
      .from('contracts')
      .select(
        'title, service_provider_id, service_receiver_id, initiator_id, counterparty_id, initiator_role'
      )
      .eq('id', contractId)
      .maybeSingle()

    const serviceProviderId =
      contractParties?.service_provider_id ??
      (contractParties?.initiator_role === 'service_provider'
        ? contractParties.initiator_id
        : contractParties?.counterparty_id ?? null)

    const serviceReceiverId =
      contractParties?.service_receiver_id ??
      (contractParties?.initiator_role === 'service_receiver'
        ? contractParties.initiator_id
        : contractParties?.counterparty_id ?? null)
    ...
    if (serviceProviderId) {
      await insertNotifications({
        ...
        body: `The service receiver funded "${contract.title}". You can now start delivering milestones.`,
      })
    }
    ...
    if (serviceReceiverId) {
      await insertNotifications({
        ...
        title:
          provider === 'coinbase_commerce' ? 'Escrow funded (crypto)' : 'Escrow funded',
        body:
          provider === 'coinbase_commerce'
            ? `Crypto escrow for "${contract.title}" confirmed on-chain.`
            : `Escrow for "${contract.title}" is funded and active.`,
      })
    }
  }
