    ...
    >
      ...
      >
        ...
        <button
          ...
          onClick={handleCopy}
          className={cn(
            'flex min-h-[44px] items-center gap-1.5 text-xs text-[hsl(var(--color-text-3))]',
      ...
      <pre
        className={cn(
          'overflow-x-auto p-4 text-[12px] font-mono leading-relaxed sm:text-[13px]',
          'bg-[hsl(220_15%_8%)] text-[hsl(var(--color-text-2))]'
        )}
      >
        {code}
      </pre>
...
function EndpointBadge({
  ...
  return (
    <div
      className={cn(
        'my-3 flex flex-col items-start gap-2 rounded-[var(--radius-md)] border border-[hsl(var(--color-border))] bg-[hsl(var(--color-surface))] p-3 font-mono sm:flex-row sm:items-center sm:gap-3'
      )}
    >
      <span className={cn('rounded px-2 py-0.5 text-xs font-bold', colors[method])}>
        {method}
      </span>
      <span className="break-all text-sm text-[hsl(var(--color-text-2))]">{path}</span>
    </div>
  )
}
...
export default function DevelopersPage() {
  ...
  return (
    <div className="flex min-h-screen flex-col gap-6 px-4 py-8 sm:px-6 lg:flex-row lg:gap-8 lg:px-8 lg:py-10">
        ...
        <div className="flex min-w-max items-center gap-2">
          {NAV_SECTIONS.map((section) => (
            <button
              ...
              className={cn(
                'min-h-[40px] flex-shrink-0 rounded-[var(--radius-md)] px-3 py-2 text-sm transition-all duration-150',
                active === section.id
      ...
      >
        ...
        {NAV_SECTIONS.map((section) => (
          <button
            ...
            onClick={() => scrollTo(section.id)}
            className={cn(
              'min-h-[44px] rounded-[var(--radius-md)] px-3 py-2 text-left text-sm transition-all duration-150',
              active === section.id
                ? 'bg-[hsl(var(--color-accent)/0.1)] font-medium text-[hsl(var(--color-accent))]'
...
webhooks:manage   - Create and manage webhook subscriptions`}
  ...
  "industry": "creative",
  "currency": "USD",
  "counterparty_email": "provider@example.com",
...
milestone.submitted   Service provider submitted a delivery
milestone.approved    Service receiver approved and payout queued

payment.released      Milestone payment sent to service provider
