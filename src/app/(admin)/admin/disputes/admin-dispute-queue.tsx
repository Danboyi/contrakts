...
const RULINGS = [
  { value: 'vendor_wins', label: 'Service Provider wins' },
  { value: 'client_wins', label: 'Service Receiver wins' },
        ...
        {selected && (
          <div className="space-y-4">
            ...
            {ruling === 'partial' && (
              <div>
                <label className="mb-2 block text-xs font-medium text-[hsl(var(--color-text-2))]">
                  Service provider receives: {vendorPct}% (
                  {formatCurrency(
                ...
                <div className="mt-1 flex justify-between text-xs text-[hsl(var(--color-text-3))]">
                  <span>0% service receiver receives all</span>
                  <span>100% service provider receives all</span>
                </div>
              </div>
            )}
            ...
            {ruling === 'client_wins' && (
              <div className="flex items-start gap-3 rounded-[var(--radius-md)] border border-[hsl(var(--color-warning)/0.2)] bg-[hsl(var(--color-warning)/0.06)] p-4">
                ...
                />
                <p className="text-sm leading-relaxed text-[hsl(var(--color-text-2))]">
                  Service receiver refunds are queued for manual handling and audit review.
                </p>
              </div>
            )}
          </div>
