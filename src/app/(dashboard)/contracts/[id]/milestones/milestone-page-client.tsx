...
import { approveMilestone, submitDelivery } from '@/lib/milestones/actions'
...
export function MilestonePageClient({
  ...
  const {
    ...
    activeMilestone,
    paidMilestones,
  } = useMilestones(contract.id, contract.milestones ?? [])
  ...
  const isClient = contract.initiator_id === currentUserId
  const isVendor = contract.counterparty_id === currentUserId
  ...
  useEffect(() => {
    if (initialMilestoneId && milestones.some((milestone) => milestone.id === initialMilestoneId)) {
  ...
  function handleSubmit() {
    ...
    if (files.length === 0 && !note.trim()) {
      setSubmitErr('Add at least one file or a note.')
      return
    }
    ...
    startTransition(async () => {
      ...
      setNote('')
      setSubmitErr(null)
      toast.success('Delivery submitted. The client has 72 hours to review.')
    })
  }
  ...
  const vendor = contract.counterparty
  ...
  return (
    <div className="mx-auto max-w-[720px]">
      ...
      <div className="mb-6 flex gap-1.5 overflow-x-auto rounded-[var(--radius-md)] border border-[hsl(var(--color-border))] bg-[hsl(var(--color-surface))] p-1">
      ...
      {selected && (
        ...
        >
          ...
          {selected.state === 'in_progress' && isVendor && (
            <div className="p-5">
              <p className="mb-4 text-sm font-medium text-[hsl(var(--color-text-1))]">
              ...
              <div className="mt-4">
                ...
                <textarea
                  placeholder="Describe what you've delivered, any notes for the client, or links to additional resources..."
                  value={note}
                  onChange={(event) => {
                  ...
                  rows={3}
                  className={cn(
                    'w-full resize-y rounded-[var(--radius-md)] border border-[hsl(var(--color-border))] bg-[hsl(var(--color-surface-2))] px-3 py-2.5 text-sm text-[hsl(var(--color-text-1))] outline-none transition-all duration-150',
                    'placeholder:text-[hsl(var(--color-text-3))] focus:border-[hsl(var(--color-accent))] focus:shadow-[0_0_0_3px_hsl(var(--color-accent)/0.12)]'
                  )}
                />
              </div>

              {submitErr && (
                <div className="mt-3 flex items-center gap-2 rounded-[var(--radius-md)] border border-[hsl(var(--color-danger)/0.3)] bg-[hsl(var(--color-danger)/0.1)] p-3">
                  <AlertTriangle size={13} className="text-[hsl(var(--color-danger))]" />
                  <p className="text-xs text-[hsl(var(--color-danger))]">{submitErr}</p>
                </div>
              )}
          ...
          {selected.state === 'in_progress' && isClient && (
            <div className="flex flex-col items-center px-5 py-10 text-center">
              ...
              <p className="max-w-xs text-xs text-[hsl(var(--color-text-3))]">
                {vendor?.full_name ?? 'The vendor'} is working on this milestone. You
                will be notified when the delivery is submitted.
              </p>
            </div>
          )}
              ...
              {isVendor && (
                <div className="flex items-center gap-2 rounded-[var(--radius-md)] border border-[hsl(var(--color-warning)/0.2)] bg-[hsl(var(--color-warning)/0.06)] p-3">
                  ...
                  <p className="text-xs text-[hsl(var(--color-text-2))]">
                    Waiting for client review. Payment auto-releases after 72 hours.
                  </p>
                </div>
              )}
            </div>
          ...
          {selected.state === 'disputed' && (
            <div className="p-5">
              ...
              <Button
                variant="secondary"
                size="sm"
                onClick={() => router.push(`/contracts/${contract.id}/dispute?milestone=${selected.id}`)}
              >
        ...
        {selected && (
          <div className="mb-2 rounded-[var(--radius-lg)] border border-[hsl(var(--color-border))] bg-[hsl(var(--color-surface))] p-4">
            <p className="text-sm leading-relaxed text-[hsl(var(--color-text-2))]">
              ...
              will be released to{' '}
              <strong className="text-[hsl(var(--color-text-1))]">
                {contract.counterparty?.full_name ?? 'the vendor'}
              </strong>
