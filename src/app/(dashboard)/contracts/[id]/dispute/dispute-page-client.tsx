  ...
  const config =
    {
      vendor_wins: {
        ...
        label: 'Vendor wins - payment release initiated',
      },
      client_wins: {
        icon: XCircle,
        tone: 'danger' as const,
        label: 'Client wins - milestone reset for re-delivery',
      },
      partial: {
        icon: Minus,
        tone: 'accent' as const,
        label: `Partial ruling - ${dispute.ruling_pct_vendor}% to vendor`,
      },
      cancelled: {
        icon: XCircle,
      ...
      {dispute.ruling === 'partial' && (
        <>
          ...
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
            {[
              { label: `To vendor (${vendorPct}%)`, amount: vendorAmount },
              { label: `To client (${100 - vendorPct}%)`, amount: clientAmount },
            ].map((entry) => (
              <div
                key={entry.label}
